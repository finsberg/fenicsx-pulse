...
This demo illustrates how to visualize the results of a cardiac simulation using `pyvista`, `matplotlib`, and `io4dolfinx`.
...
3.  **Regional Analysis**: Compute average stress and strain in the **17-segment AHA model** {cite}`cerqueira2002standardized` over time.
4.  **Plotting**: Generate time-trace plots of these regional metrics.

**Requirements:**
* `pyvista`, `matplotlib`, `io4dolfinx`
...
```python
timestamps = io4dolfinx.read_timestamps(
    checkpoint_filename,
    comm=comm,
...
**17 AHA segments** {cite}`cerqueira2002standardized`.
