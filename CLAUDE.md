...
- **`active_model.py` / `active_stress.py`** — active contraction models (`Passive`, `ActiveStress`, `FrankStarlingActiveStress`, `StabilizedActiveStress`), same `strain_energy`/`S`/`P` interface as materials, driven by a `Variable` activation parameter you update over time (`Ta.assign(...)`).
  - `ActiveStress` takes a `formulation` (`ActiveStressFormulation`): `invariant` (the default and historical behaviour, `Ψ = ½Ta(I4f−1)` ⟹ `P = Ta·F f0⊗f0`) or `stretch` (`Ψ = Ta(λ−1)` ⟹ `P = Ta·F f0⊗f0/|F f0|`). The two differ by a factor of λ, so switching is a modelling change, not a refactor — pick `stretch` when `Ta` comes from a model whose stiffness is defined against λ. `stretch` requires `eta == 0`.
  - `StabilizedActiveStress` adds the consistent stabilization term of Regazzoni & Quarteroni (2021), `Ψ = Ta·Δλ + ½Ka·Δλ²` with `Δλ = λ − λ_prev`. Use it whenever `Ta` comes from an *external* force-generation solver: the plain staggered scheme is not merely inaccurate but non-convergent once active stiffness exceeds passive stiffness, and refining `dt` makes it worse. Callers must call `update_prev(u)` after each solve, with the same λ that drove the force-generation model.
  - Both are covered by `tests/test_stabilized_active_stress.py`. Its two load-bearing tests check that the stabilization vanishes at its own fixed point (so it biases nothing) and that the assembled Jacobian both matches finite differences *and* changes when `Ka` does (so Newton actually sees it) — a stabilization term Newton cannot see is inert. Keep both properties if you touch this code.
